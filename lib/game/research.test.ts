...
test("canAffordResearch is true only when both resources cover the cost", () => {
  ...
  assert.equal(canAffordResearch({ gold: 100, food: 0, iron: 0, researchResource: 50 }, cost), true);
  assert.equal(canAffordResearch({ gold: 99, food: 0, iron: 0, researchResource: 50 }, cost), false);
  assert.equal(canAffordResearch({ gold: 100, food: 0, iron: 0, researchResource: 49 }, cost), false);
});

test("upgradeResearch raises the chosen category by exactly one level", () => {
  const resources: FactionResources = { gold: 1000, food: 0, iron: 0, researchResource: 1000 };
...
test("upgradeResearch deducts exactly the cost from resources", () => {
  const resources: FactionResources = { gold: 1000, food: 42, iron: 0, researchResource: 1000 };
...
test("upgradeResearch throws once a category is already at the max level", () => {
  ...
  const resources: FactionResources = { gold: 999999, food: 0, iron: 0, researchResource: 999999 };
...
test("upgradeResearch never leaves a resource negative across many upgrades", () => {
  ...
  let resources: FactionResources = { gold: 100000, food: 0, iron: 0, researchResource: 100000 };
