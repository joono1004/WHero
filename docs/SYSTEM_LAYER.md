...
- 지형 카테고리 목록(예: 평원/숲/언덕/산/습지/해안 등)이 아직 시스템 쪽과
  ...
  정해야 합니다.
- **영웅 초상화/그래픽 (2026-08-06 갱신 — 아래는 2026-07-27 시점 기록이라
  낡았습니다)**: 이제 카드에 실제로 초상이 들어갑니다. `HeroDefinition`에
  이미지 필드를 추가하는 대신, `app/game/screens/HeroSelectScreen.tsx`의
  `HERO_PORTRAIT: Partial<Record<HeroId, string>>` 상수에 `heroId -> 경로`를
  직접 등록하는 방식으로 함 (아직 극소수 영웅만 실제 아트가 있어서
  `HeroDefinition` 스키마까지 건드릴 필요가 없었음 — 아트가 더 늘어나면
  그때 필드로 승격을 고려해도 됨). 프레임/아트 스펙은 아래 "영웅 카드 시스템
  재작업" 항목 참고.

## 로비 화면 현재 구성 (스냅샷, 2026-08-07 기준)

아래에서 하나씩 결정된 과정은 "결정 사항 로그"의 여러 2026-08-06 항목에
흩어져 있는데, 매번 그걸 순서대로 읽지 않아도 지금 화면이 어떻게
생겼는지 바로 알 수 있도록 별도로 정리합니다. 코드 출처는
`app/game/screens/GameLobbyScreen.tsx`이고, 이 파일이 바뀌면 이 스냅샷도
같이 갱신해야 합니다.

**헤더** (`grid-cols-[auto_1fr_auto]` 3분할, 한 줄):
- **왼쪽**: 대표 영웅 초상(원형, 클릭 시 "아직 준비 중" 안내 - 지금은
  항상 `entries[0]` = 최초 선택한 영웅의 초상, 나중에 영웅을 더 모으면
  직접 고르고 테두리도 고르는 시스템으로 확장 예정) + 세력명·"정복 세계
  N" 뱃지(1줄) + 칭호(2번째 줄, 지금은 항상 "신입 지휘관" 고정 문구 -
  실제 칭호 시스템 없음).
- **가운데**: "Hero Story" 타이틀, 크고 굵게 정중앙.
- **오른쪽**: 자원 칩 목재/철광/금화/보석 순(K단위 표기) + 보석 옆 `[+]`
  구매 버튼(placeholder) + ✉️(우편)/🔔(알림)/⚙(설정)/✕(닫기) 버튼. 우편·
  알림·`[+]`는 눌러도 "아직 준비 중" 안내만 뜸(실제 시스템 없음).

**본문** (좌측 사이드바 + 우측 메인, 2컬럼):
- **왼쪽 사이드바** (`w-24`, 세로):
  - 세로 메뉴 6개(영웅/병사/가방/연구/순위/연맹(예정)) - 아이콘+라벨
    한 줄짜리 버튼, 목록이 넘치면 `overflow-y-auto`로 스크롤(항목이
    늘어도 레이아웃 안 바뀌게 하려는 의도). **영웅**과 **연구**만 실제
    화면(`HeroRosterScreen`/`ResearchScreen`)으로 연결되고, 나머지
    (병사/가방/순위/연맹)는 "아직 준비 중" 안내만. `HeroRosterScreen`은
    2026-08-07부터 목록 단독 화면이 아니라 [영웅리스트][영웅정보][보물]
    [가방] 4열이 한 화면에 같이 있는 통합 화면 - 아래 "영웅 클릭 시"
    항목 참고. (여기 사이드바 메뉴의 "가방" 항목은 이 4열 화면의 가방
    칸과 별개 항목이라 이름이 겹치는데, 사이드바 쪽은 아직 클릭해도
    "아직 준비 중" 안내만 뜨는 placeholder - 실제 가방 UI는 영웅 화면
    안에만 있음.)
  - 메뉴 아래 **광고배너** 자리(점선 박스, "광고" 텍스트, placeholder) -
    실제 광고 SDK 없음.
  - 그 아래 **채팅창** 자리(배경 투명) - placeholder 메시지 2줄 +
    비활성화된 입력창 1줄. 실시간 채팅은 서버/네트워킹이 필요한 큰
    기능이라 이번엔 UI 자리만 예약, 실제 송수신 기능 없음.
- **오른쪽 메인**:
  - **세계 선택 레일** ("공략할 세계를 선택하세요") - 정복 완료한 세계
    히스토리 칩(왼쪽, 오래된 순) + 새 후보 카드(오른쪽, 처음엔 1개,
    이후 정복할 때마다 서로 다른 지형 2개) 순서로 가로 배열, `‹`/`›`
    버튼 + 드래그 스크롤. 클리어한 세계 중 영주가 없는 곳을 클릭하면
    `GovernorAppointScreen`으로 이동해 영주 임명 가능(영웅 선택 + 🎲
    이름 랜덤 지정). 새 후보 카드는 "이 세계로 진출" 버튼으로
    `HeroEnlistScreen`(출전 영웅 선택) 진입.
  - **출전 영웅 5슬롯** (세계 레일 아래) - `deploymentPriority`(★)가
    켜진 영웅을 우선순위 순으로 채워서 보여주는 것으로, 실제 어떤
    영웅이 세계에 나가는지와는 별개(그건 `HeroEnlistScreen`에서 그때그때
    고름) - "다음에 출전시킬 생각인 주전"을 한눈에 보여주는 용도. 채워진
    슬롯 클릭 → 그 영웅이 미리 선택된 채로 `HeroRosterScreen`(통합
    화면) 진입, 빈 슬롯 클릭 → 같은 화면을 기본 선택값으로 진입.

**아직 없는 것 / 다음에 결정할 것**: 실시간 자원 생산(목재/철광/금화/
보석은 항상 0), 칭호 획득 로직, 대표 영웅 직접 선택 + 테두리 커스터마이즈
피커, 우편·알림 시스템, 광고 SDK 연동, 채팅 서버, 병사/가방/순위/연맹
화면 자체.
...
- **내정 표시 → 특기(패시브) 시스템, 스킬(액티브) 시스템 신설 (2026-08-06)**:
  사용자가 영웅 카드를 화면 단위로 검토하는 과정에서 "내정" 등급 표시(SS~D)
  개념을 없애고, 이름 있는 고정 효과 패시브 "특기"로 교체 → 이어서 "특기는
  상시 패시브, 스킬은 전투 중 사용하는 액티브"로 한 단계 더 분리 요청.
  이번 패스는 **데이터 + 화면 표시만** — 어느 쪽도 아직 `combat.ts`/
  `movement.ts`에 수치로 연결되지 않음(설명 텍스트만 존재).
  - `lib/game/hero-trait.ts` (신규): `HERO_TRAIT_CATALOG`(id → `{name,
    effect}`), `HeroTraitId`, `MAX_HERO_TRAITS = 5`. 기존 `hero.ts`의
    빈 `TraitKind` 플레이스홀더("charge"/"magic", 아무 데도 안 쓰이던 것)를
    이걸로 완전히 대체.
  - `lib/game/hero-skill.ts` (신규, `hero-trait.ts`의 자매 모듈):
    `HERO_SKILL_CATALOG`, `HeroSkillId`, `MAX_HERO_SKILLS = 2`.
  - `HeroDefinition`(`hero-definition.ts`)에 `traits: HeroTraitId[]`,
    `skills: HeroSkillId[]` 필드 추가. **`domesticSpecialties`(구 "내정")
    필드는 그대로 유지** — `hero-assignment.ts`의 도시 산출/생산/방어/회복
    보너스 함수들이 실제로 이 필드를 읽어서 동작하는, 이미 완성되고
    테스트된 로직이라 표시 이름만 바꾸고 내부 로직은 안 건드림(주석으로
    명시해둠).
  - 최초 배치: 감녕 `traits:["talent"] skills:["charge"]`, 위연
    `traits:["talent"] skills:[]`, 서서 `traits:["trade","farming"]
    skills:[]`, 관우 `traits:["talent","farming"] skills:["ironwall"]`
    (관우의 "철벽"은 원래 특기 후보였다가 "전투 중 쓰는 액티브"라는 사용자
    피드백으로 스킬로 재분류), 조운/제갈량은 특기만 있고 스킬 없음. 사용자가
    준 예시 목록(인재/벌목/농사/상인/돌격/무쌍/쌍수/철벽/수군/산악/원사)
    중 돌격·철벽은 스킬로, 나머지는 특기로 확정 — "난사"(주변 적 병사 모두
    공격)가 스킬로 새로 추가됨. 나머지 특기 후보 중 스킬로 옮길 게 더
    있는지는 아직 사용자 확인 대기 중.

- **영웅 카드(`HeroSelectScreen.tsx`) 레이아웃 재작업 (2026-08-06, 위 특기/
  스킬 작업과 같은 세션에서 여러 라운드로 진행)**: Codex가 영웅 초상 아트를
  만들 때 참고할 수 있게 최종 레이아웃과 아트 스펙을 정리합니다.
  - **카드 구조 (위→아래)**: `이름 + [등급·유형] 배지` (왼쪽) / `병과: 값`
    (오른쪽) 한 줄 → `[초상 96x96] + 통솔/무력/지력/체력/매력 등급 그리드`
    (2열, 값은 라벨 뒤 남은 공간에 중앙 정렬) → `특기` 라벨 + 보유 특기
    수만큼 채워지는 고정 5칸 슬롯 바(빈 칸은 🔒) → 설명(3줄 클램프).
    **스킬은 이 카드에 안 보입니다** — `HeroDetailScreen.tsx`에만 표시 중
    (카드에 어떻게 넣을지는 사용자가 "고민 좀 해볼게"로 보류함, 스킬 슬롯
    UI는 아직 미확정).
  - **초상 프레임 스펙 (Codex 아트 제작 시 참고)**: 고정 **96×96px 정사각형**
    (`HeroSelectScreen.tsx`의 `HERO_PORTRAIT_FRAME_PX` 상수), `object-fit:
    cover`로 렌더링 — 정사각형이 아닌 원본도 동작은 하지만 중앙 기준으로
    잘리므로, 인물의 얼굴/핵심 구도가 중앙 근처에 있는 게 안전합니다. 기존
    위연 샘플(`public/art/heroes/wei-yan-classic-portrait-v3.webp`, 512×512
    webp)이 이 스펙의 기준 — 새 영웅 초상도 정사각형·512×512 이상·webp로
    맞춰서 `public/art/heroes/`에 추가하고 `HeroSelectScreen.tsx`의
    `HERO_PORTRAIT` 맵에 `heroId -> 경로`만 등록하면 카드에 바로 반영됩니다.
    등록 안 된 영웅은 🧑 이모지 placeholder로 자동 대체.
  - **프레임 크기를 명시적으로 고정한 이유**: 원래는 `items-stretch`로
    "능력치 그리드 높이에 맞춰 초상 박스도 늘어나게"만 해뒀는데, 이미지가
    있는 위연 카드(94×94 정사각형, `<img>`의 내재 종횡비 때문에 실제로는
    안 늘어남 — 퍼센트 높이를 가진 `<img>`가 "definite height"가 없는
    stretch된 flex item 안에서는 스펙상 종횡비로 폴백하는 flexbox 버그성
    동작)와 아직 이모지뿐인 감녕/서서 카드(96×76, 순수 그리드 컨텐츠
    높이대로 늘어남)의 프레임 크기가 서로 달라져서 발견됨. 지금은 폭/높이
    둘 다 96px로 고정해서 아트 유무와 무관하게 세 카드 모두 동일한 프레임.
  - 그 외 진행된 세부 조정: 등급 값(S/A/B/C/D)을 라벨 뒤 남은 공간에서
    중앙 정렬(기존엔 justify-between으로 열 오른쪽 끝에 붙어있었음), 병과
    정보가 매력 옆 좁은 칸에서 잘리는 문제 → 헤더 줄(이름 옆이 아니라 카드
    오른쪽)로 이동, 등급·유형 배지는 이름 바로 옆으로 이동, 카드 안쪽 여백
    축소(스킬 표시 제거 + 여백 축소로 hero-select 화면의 세로 스크롤이
    완전히 사라짐 — 3장 카드가 844×390 프레임 안에 스크롤 없이 다 들어감).
  - 검증: `pnpm run build`/`npx tsc --noEmit`/`pnpm run test:game`
    (312/312, 특기·스킬 검증 테스트 각 1개씩 추가)/린트(기존 사전 존재
    문제 10개 제외 클린) 매 라운드마다 통과 확인, Playwright로 매 변경마다
    스크린샷 검증.

- **영웅 카드에 스킬 정보 팝업 신설 (2026-08-06, 위 특기/스킬 시스템 작업
  바로 다음 라운드)**: 카드에는 스킬을 아예 안 보여주기로 했던 결정(바로
  위 "영웅 카드 레이아웃 재작업" 항목 참고) 이후, 사용자가 대신 팝업 UI를
  요청함 - 화면 전환이 아니라 현재 화면 위에 뜨는 모달.
  - **[스킬] 버튼**: "특기" 라벨 오른쪽에 위치, **항상 표시**(스킬 보유
    여부와 무관 - 처음엔 조건부였다가 "스킬이 없어도 버튼은 있어야 한다"는
    사용자 피드백으로 상시 노출로 변경). 폰트는 "특기" 라벨과 동일 크기
    (0.75rem) - 처음엔 더 작은 10px로 만들었다가 세로 패딩까지 겹쳐서
    28px짜리 버튼이 되는 바람에 카드 목록에 스크롤이 다시 생겼음 → 폰트를
    특기 라벨과 맞추고 lineHeight/패딩을 눌러서 19px로 줄여 해결.
  - **카드 자체를 `<button>`에서 `<div role="button" tabIndex={0}>`로 전환**:
    스킬 버튼이 카드 안에 중첩된 실제 `<button>`이라, `<button>` 안에
    또 다른 인터랙티브 엘리먼트를 넣을 수 없는 HTML 제약 때문에 필요했던
    변경. 클릭/키보드(Enter, Space) 동작은 동일하게 유지, 스킬 버튼
    클릭은 `stopPropagation`으로 카드 선택(onSelect)까지 같이 발동되는 걸
    막음.
  - **팝업(`SkillModal`)**: `position: absolute; inset: 0`로 카드가 아니라
    **게임 화면 전체**(DeviceFrame의 `position: relative` 박스까지 escape
    - `position: fixed`를 썼으면 데스크톱 미리보기에서 844×390 박스를
      벗어나 브라우저 전체를 덮었을 것이므로 의도적으로 fixed 대신
      absolute 선택)를 덮는 배경(반투명 검정) + 중앙 카드로 구현. 배경
      클릭 또는 우측 상단 ✕로 닫힘, 팝업 내부 클릭은 전파 차단.
  - **스킬 슬롯은 `MAX_HERO_SKILLS`(2)개 고정 표시** - 특기 5칸 슬롯 바와
    같은 패턴. 스킬이 하나뿐이거나 하나도 없어도 항상 2줄이 보이고,
    비어있는 자리는 애니메이션 칸에 🔒, 이름 자리에 "미확인"으로 표시.
  - **스킬 데이터 스키마 확장**: `lib/game/hero-skill.ts`의
    `HERO_SKILL_CATALOG` 각 항목이 기존 `effect` 한 줄에서 `summary`(짧은
    수치 요약, 예: "공격력 20% 추가 데미지, 방어력 10% 감소")와
    `description`(긴 서술형 설명) 두 필드로 분리됨 - 팝업이 두 줄을 다
    보여줘야 해서 필요해진 변경. **돌격(charge)의 수치(20%/10%/3턴)는
    사용자가 확정한 공식 값**으로, 이전 플레이스홀더("공격 2배, 방어
    1.5배 감소")를 대체함. **철벽/난사는 아직 실제 수치가 없어서** 기존
    한 줄 effect를 요약/설명으로 임시 분리해둔 상태 - 실제 밸런스 수치가
    나오면 교체 필요.
  - **애니메이션 자리**: 스킬 사용 시 연출(전투 중)을 넣을 자리로, **지금은
    빈 placeholder일 뿐** - "애니메이션 (준비 중)" 텍스트 또는(잠긴 슬롯)
    🔒 아이콘만 있고 실제 애니메이션/이펙트는 전혀 없음. **Codex와 나중에
    같이 채울 예정** (사용자가 여러 차례 명시). 박스 크기는 카드 이름/요약/
    설명 3줄 높이에 맞춰 자동으로 늘어나다가, 사용자 요청으로 자연 높이
    (~91.5px) 대비 +10%인 **고정 100px**로 확정(잠긴 슬롯도 동일하게
    100px 확보 - 그전엔 짧은 "미확인" 한 줄 때문에 훨씬 낮았음). 팝업 전체
    가로폭도 사용자가 두 차례 요청해 320px → 480px → **550px**로 넓어졌고,
    애니메이션 박스 폭(72→108→120px)과 요약/설명 폰트 크기(text-xs→
    text-sm)도 그때마다 비율 맞춰 같이 조정.
  - 검증: 매 라운드 `pnpm run build`/`npx tsc --noEmit`/`pnpm run
    test:game`(312/312) 통과, Playwright로 스킬 0개/1개 영웅 둘 다
...
- **로비 화면 재배치 1차 초안 + 영주 임명 시스템 신설 (2026-08-06, task 13
  착수)**: 사용자가 로비 화면을 여러 라운드에 걸쳐 요구사항만 먼저
  정리("이해했니?"로 매번 확인받으며 진행)한 뒤, "연구 같은 상세 시스템은
  나중에 정리하고 일단 화면부터 만들어봐"라는 방향으로 1차 구현을 요청함.
  용어를 이 시점부터 **"세계"로 통일**(이전 "N번째 세계" 표기도 전부
  "세계 N"으로 정리).
  - **`FactionResources`에 `wood`/`gem` 필드 추가** (`faction.ts`) - 로비
    헤더에 표시할 "금화/목재/광석/보석" 4종 자원 중 금화(gold)/광석(iron)은
    기존 필드를 재사용하고, 목재/보석은 새 필드. **실시간 생산 로직은
    이번 패스에 포함 안 됨** - 항상 0으로 시작하고, 나중에 영주 배치
    생산이 붙으면 자동으로 반영되는 자리만 마련. gem은 영웅 뽑기(가챠)용
    프리미엄 화폐로도 쓰일 예정(사용자 방향, 시스템 자체는 완전히 미착수).
    기존 FactionResources 리터럴을 쓰던 테스트 10곳 이상(`city-actions.
    test.ts`/`city.test.ts`/`research.test.ts`/`world-entry.test.ts`)에
    `wood: 0, gem: 0` 추가.
  - **`ClearedWorldRecord.name: string | null` 필드 추가** (`world.ts`) -
    "영주 임명시 세계 이름을 정할 수 있어야 한다"는 요구사항 반영.
    `save.test.ts`/`world-entry.test.ts`의 리터럴 5곳에 `name: null` 추가.
  - **`lib/game/governor.ts` 신규**: `appointGovernor(save, worldId, heroId,
    name?)`. `hero.ts`의 `appointHeroAsGovernor`(이미 있었음 - hero의
    assignment만 바꾸는 저수준 함수, `world-entry.test.ts` 픽스처용으로만
    쓰이고 있었음)를 감싸서, 실제 UI 진입점이 필요로 하는 검증(월드가
    실제로 클리어된 상태인지, 이미 영주가 있는지, 영웅이 유휴 상태
    (`isHeroEnlistable`)인지)과 `ClearedWorldRecord.governorHeroId`/`name`
    동시 갱신을 더함. 이름을 안 주거나 공백만 주면 기존 이름을 유지(첫
    임명 전이면 계속 이름 없음 상태). 테스트 9개 신규
    (`governor.test.ts`) - 정상 임명, 이름 지정, 공백 이름 처리, 4가지
    실패 케이스(없는 세계/이미 영주 있음/없는 영웅/유휴 아닌 영웅), 순수
    함수(원본 미변경) 검증.
  - **`app/game/heroPortraits.ts` 신규**: `HeroSelectScreen.tsx`에 있던
    `HERO_PORTRAIT`/`HERO_PORTRAIT_FRAME_PX`를 공용 모듈로 분리 - 로비의
    영웅 리스트도 같은 초상 맵을 쓰게 됐기 때문(카드는 96x96, 로비
    리스트는 28x28로 더 작게 표시하지만 소스는 같음).
  - **`app/game/screens/GovernorAppointScreen.tsx` 신규**: 영주로 임명할
    유휴 영웅을 고르는 화면(`HeroEnlistScreen`과 같은 카드 그리드, 다만
    다중 선택이 아니라 단일 선택) + 세계 이름 입력(`FactionNameScreen`과
    같은 🎲 무작위 생성 패턴 재사용, 다만 세력명과는 다른 지역/영지
    느낌의 별도 단어 목록 `RANDOM_WORLD_NAMES`).
  - **`GameLobbyScreen.tsx` 재작업**:
    - 헤더: `Lv.{faction.level}` → **`정복 세계 {clearedWorlds.length}`**,
      그 아래 새 자원 줄 `금화 · 목재 · 광석 · 보석` 추가(기존 푸터의
      `금 · 식량 · 유산` 줄은 그대로 유지 - 서로 다른 자원 계통이라 겹치는
      부분(금)이 있다는 점은 사용자에게 별도로 알릴 것).
    - 영웅 리스트: 이름 첫 글자 원형 뱃지 → **28x28 초상 프레임**(아트
      없으면 🧑 placeholder, `HERO_PORTRAIT` 공유). 영주 라벨은 이제
      "세계 N 영주" 대신 **커스텀 이름이 있으면 그 이름 우선**
      ("청화지 영주" 등).
    - 세계 레일: 클리어한 세계 칩이 이제 **클릭 가능**(미임명 상태면
      `GovernorAppointScreen`으로 진입) + **지형 아트 placeholder 박스**
      (Codex 몫, 지금은 "지형" 텍스트만 있는 점선 박스 - 티어가 높을수록
      살짝 커지게 해서 "정복 세계가 많을수록 큰 세계가 보인다"는 방향을
      아트 없이도 시각적으로 어느 정도 반영, `TerrainArtPlaceholder`
      컴포넌트). 후보 카드도 같은 placeholder 박스 적용 + "N번째 세계"
      표기를 "세계 N"으로 통일. 레일 좌우에 **`‹`/`›` 버튼** 추가
      (`scrollBy`), 그리고 세계를 새로 클리어할 때마다 레일이 자동으로
      맨 오른쪽(최신 후보 쪽)으로 스크롤되도록 `useEffect` 추가 - 사용자가
      "새로운 세계 후보가 중앙 근처에 기본적으로 보여야 한다"고 한 요구를
      정확히 중앙 정렬은 아니지만 "스크롤 없이 바로 보이게"로 우선
      구현(추가 다듬기 여지 있음, 아래 참고).
    - 첫 클리어 전엔 후보 1개(`generateInitialMapCandidates`), 클리어
      후엔 서로 다른 지형 2개(`generateNextMapCandidates`)가 뜨는 기존
      로직은 전혀 안 건드림 - map-candidates.ts가 이미 사용자가 원하는
      그대로였음(사용자에게도 이 사실을 먼저 알리고 진행함).
  - **아직 미구현/임시로 남긴 것** (사용자가 명시적으로 "연구 같은 상세는
    나중에"라고 한 부분과 겹침):
    - 영주 배치의 **실시간 자원 생산 로직 자체** (틱 주기, 오프라인 적립
      여부, 공식) - `FactionResources`에 필드만 마련.
    - 연구 카테고리 확장(영웅 병과 업그레이드/전투 버프 등) - 기존 연구
      화면·버튼(`ResearchScreen.tsx`, 로비 푸터의 "연구" 버튼)은 그대로
      두고 손 안 댐 - 사용자가 요청한 "적당한 위치에 버튼"은 이미 있던
      버튼으로 충족.
    - **보석/가챠 시스템 전체** - 완전히 새로운 시스템, 이번엔 헤더에
      숫자 자리만 마련.
    - 이미 임명된 영주를 다른 영웅으로 교체하는 흐름 - `ClearedWorldChip`
      이 영주가 있으면 비클릭 정보 표시로만 남김.
    - 레일 자동 스크롤이 "정확히 중앙" 정렬은 아님(맨 오른쪽으로 스크롤) -
      사용자의 "새로운 세계는 중앙보다 살짝 오른쪽" 요구를 완전히
      구현하려면 후보 카드 개수/폭 기준의 스크롤 위치 계산이 더 필요함.
  - 검증: `pnpm run build`/`npx tsc --noEmit`/`pnpm run test:game`
    (321/321 - governor.test.ts 9개 신규)/린트(기존 사전 존재 문제 10개
    제외 클린) 통과. Playwright로 전체 흐름 실측 확인: 첫 로비(후보
    1개) → 맵 진입 → 테스트 버튼으로 정복 → "정복 완료" → 로비 복귀(정복
    세계 1, 서로 다른 지형 후보 2개 노출) → 클리어 칩 클릭 → 🎲로 세계
    이름 생성 → 영웅 선택 → 임명 → 칩이 이름/영주 상태로 갱신 + 영웅
    리스트에 "OO 영주" 라벨까지 전부 확인.

- **로비 화면 재배치 2차: 헤더/푸터 정리 + 메뉴 배치 구조 확정
  (2026-08-06, 위 1차 초안 바로 다음 라운드)**: 사용자 피드백 "화면 자체가
  게임스럽지 않다"에 대응해 먼저 순수 시각 보정(지형 오브 글로우,
  그라데이션 카드, 자원 아이콘 칩, 등급색 초상 링 - `TerrainOrb`,
  `RESOURCE_CHIPS`, `GRADE_COLOR` 도입)을 한 라운드 거친 뒤, 이어서 사용자가
  헤더/푸터 배치와 "로비에서 실제로 쓸 기능을 어떻게 배치할지"를 구체적으로
  요청함.
  - **헤더 1줄 통합**: 기존엔 이름/타이틀/버튼 줄 + 자원 줄, 총 2줄이었던
    걸 `grid-cols-3` 한 줄로 통합 - [세력명+정복세계 뱃지](왼쪽) / "HERO
    STORY"(정중앙) / [자원 칩들 + ⚙✕ 버튼](오른쪽).
  - **푸터에서 부대/도시/연구/금·식량·유산 텍스트 전부 제거**: "로비에
    필요 없다"는 사용자 판단. `ScreenShell`은 `footer` prop을 안 넘기면
    그 영역 자체를 렌더링 안 하므로(`{footer && <div>...}`), 여백 조정을
    위한 별도 계산 없이 flex-1 콘텐츠 영역이 자동으로 넓어짐 - 이후 확인차
    Playwright로 `scrollHeight === clientHeight`(스크롤 없음)까지 실측.
  - **메뉴 배치 논의 → 실제 적용**: 사용자가 "이 장르 게임에서 메뉴를
    보통 어떻게 배치하냐"고 물어서, 가로형(844×390) 게임 특성상 하단
    탭바보다 **얇은 아이콘 한 줄**이 낫다는 점, 그리고 "영웅" 관리처럼
    깊은 조작은 상시 리스트보다 **버튼으로 진입하는 전용 화면**이 낫다는
    점을 추천 → 사용자가 그대로 적용 요청, 단 메뉴 항목이 5개보다 더
    늘어날 수 있다는 점을 감안해 **하단 왼쪽부터 시작하는 확장 가능한
    아이콘 메뉴**로 요청함.
    - **`GameLobbyScreen.tsx`의 `<aside>` 사이드바 완전 제거**, 대신
      `app/game/screens/HeroRosterScreen.tsx` 신규 - 정렬 버튼(등급/유형/
      레벨순) + 전체 로스터 리스트를 그대로 옮김(로직 변경 없음, 위치만
      이동). `[영웅]` 메뉴 버튼을 눌러야 진입.
    - 로비 자체에는 대신 **"출전 영웅" 5슬롯 컴팩트 바**만 남김 -
      `deploymentPriority`(★ 표시)가 켜진 영웅을 우선순위 순으로 채우고,
      빈 슬롯은 특기/스킬 슬롯과 같은 🔒 패턴으로 표시(눌러도
      `HeroRosterScreen`으로 이동). 채워진 슬롯을 누르면 바로
      `HeroDetailScreen`으로 이동 - 로스터 화면을 거치지 않고 원터치로
      확인 가능.
    - **`MENU_ITEMS` 배열 기반 하단 좌측 메뉴 바 신설**: 연구(📜)/상점(🏪)/
      영웅(🛡️)/병사(⚔️)/아이템(🎒), `overflow-x-auto` + 왼쪽 정렬(그냥
      `justify-between`을 안 씀)이라 항목이 늘어나면 배열에 추가만 하면
      됨(레이아웃 재작업 불필요). 연구/영웅만 실제 화면 연결, 나머지는
      "아직 준비 중인 기능입니다" 안내만.
    - **`showResearch`/`updateFaction` 되살림**: 직전 라운드에서 연구
      버튼을 통째로 지우면서 죽은 코드까지 같이 지웠었는데("연구는 나중에
      다시 얘기하자"는 전제였음), 이번에 "연구 버튼 자리는 여기가 맞는 것
      같다"로 결론이 나서 그대로 복원.
  - 검증: `pnpm run build`/`npx tsc --noEmit`/`pnpm run test:game`
    (321/321, 화면 리팩터라 게임 로직 변경 없음)/린트(기존 문제 10개
    제외 클린) 통과. Playwright로 헤더 한 줄 배치, 5슬롯 바(장포가 기본
    ★라 첫 슬롯에 채워짐 + 나머지 4칸 🔒), `[영웅]` 버튼 → 로스터 화면
    진입, 채워진 슬롯 클릭 → `HeroDetailScreen` 직행, `[상점]` 클릭 →
    "아직 준비 중" 알림까지 전부 스크린샷/동작 확인.
- **로비 헤더 사용자 지정 레이아웃으로 재배치 (2026-08-06, 위 게임스럽게
  패스 바로 다음 라운드)**: 사용자가 ASCII 목업과 실제 참고 이미지를
  두 차례에 걸쳐 첨부하며 헤더 구조를 구체적으로 지정함 - 최종적으로
  `grid-cols-[auto_1fr_auto]` 3분할로 재작업.
  - **왼쪽(auto)**: **대표 영웅 초상** 원형 배지(클릭 가능, 지금은
    "아직 준비 중" 안내만) + 오른쪽에 세력명·"정복 세계 N" 뱃지(1줄) /
    칭호(2번째 줄, 지금은 항상 "신입 지휘관" placeholder - 실제 칭호
    획득/부여 로직은 아직 미설계) 텍스트 블록.
    - **대표 영웅 결정 규칙**: 사용자 확답 - "처음에는 처음 선택한
      영웅의 초상이고, 나중에 영웅을 많이 모으게 되었을때 대표영웅초상
      클릭해서 소유한 영웅을 고를수 있게끔 할꺼야, 그때 테두리도 고를수
      있게끔 하는 시스템이야". 지금은 전자만 구현: `entries[0]` (=
      `save.heroes[0]`) 사용 - `buildHeroListEntries`가 `save.heroes`
      순서를 그대로 보존(flatMap, 정렬 없음)하고 `heroes`는 append-only라
      `heroes[0]`이 곧 "최초 선택 영웅"을 안정적으로 가리킴. 나중에
      영웅을 더 모으면 이 배지를 눌러 대표 영웅 + 테두리를 직접 고르는
      피커 시스템을 만들 예정이나, 그 선택 상태를 저장할 필드가 아직
      `SaveGame`에 없음 - **명시적으로 이번 라운드 범위 밖, 미착수**.
  - **가운데(1fr)**: "Hero Story" 타이틀 - 직전 라운드에서 8px로 줄여
    좌측에 붙여뒀던 걸 다시 크고 굵게, `justify-self-center`로 정중앙에
    배치(요청: "게임명은 좌측이로 이동해도 됨" → 이후 이미지 목업에서
    번복되어 다시 중앙 대형 타이틀로 확정).
  - **오른쪽(auto)**: 자원 칩 순서를 목재/철광/금화/보석으로 재배치(기존
    "광석" 라벨 → "철광"으로 변경, `RESOURCE_CHIPS` 데이터 갱신) +
    보석 옆 `[+]` 구매 버튼(placeholder, 상점/과금 미구현) + 신규
    ✉️(우편)/🔔(알림) 버튼(둘 다 신규 개념, 시스템 없음 - "아직 준비 중"
    안내만) + 기존 ⚙(설정)/✕(닫기).
  - 검증: `npx tsc --noEmit` 클린, `pnpm run build` 성공, `pnpm run
    test:game` 321/321, 린트 기존 문제 10개 제외 클린. Playwright로
    위연(실제 초상 아트 보유 영웅) 선택 후 헤더 스크린샷 확인 - 대표
    영웅 초상에 실제 아트가 뜨는 것, "Hero Story"가 정중앙 대형으로
    보이는 것, 자원 줄/우편/알림/설정/닫기가 한 줄에 정상 배치되는 것,
    전체 화면 스크린샷에서 오버플로우 없는 것까지 확인.
- **로비 화면 전체 구조를 사용자 와이어프레임 기준으로 재배치 (2026-08-06,
  헤더 재배치 바로 다음 라운드)**: 사용자가 로비 전체 화면 구조를 담은
  와이어프레임 이미지를 첨부(헤더는 유지, 그 아래를 좌측 세로 메뉴 + 중앙
  큰 "세계" 영역 + 좌하단 광고배너/채팅창 + 우하단 출전영웅 슬롯으로 재구성)
  하며 "내가 생각하는 화면 구조야"라고 요청. 큰 구조 변경이라 진행 전
  `AskUserQuestion`으로 3가지만 확인: (1) 채팅창 범위 → **빈 UI 자리만
  예약**(서버/네트워킹 없는 로컬 placeholder, 두 줄 메시지 + 한 줄
  비활성 입력창), (2) 순위/연맹/광고배너 범위 → **버튼/자리만, "아직 준비
  중" 안내**, (3) 가운데 "세계" 영역 → **기존 세계 선택 레일(정복 이력 +
  후보 카드, `‹`/`›` 이동)을 그대로 확대 배치**. 셋 다 사용자가 추천안대로
  확정.
  - **좌측 세로 메뉴로 전면 교체**: 이전 라운드의 하단 좌측 가로 메뉴 바
    (`ScreenShell`의 `footer` prop)를 없애고, `MENU_ITEMS`를 영웅/병사/
    가방/연구/순위/연맹(예정) 6개로 갱신(기존 "상점" 삭제, "아이템"→
    "가방"으로 개명 - 와이어프레임 문구 그대로) + `overflow-y-auto`
    세로 컬럼으로 렌더링. `MenuBarButton`도 세로 사이드바에 맞게
    아이콘-위/라벨-아래 스택 → **아이콘+라벨 한 줄 가로 배치**로 재설계
    (스택형은 6개가 사이드바 높이에 못 들어가 3개만 보이고 스크롤이
    필요했음 - 한 줄형으로 바꾸니 6개가 스크롤 없이 다 들어감).
    - **globals.css 버튼 함정 재발**: 처음엔 Tailwind `px-1.5 py-1
      rounded-md`로 패딩/모서리를 주고 나머지만 인라인 `style`로
      덮었는데, 실제로는 각 버튼이 예상보다 훨씬 크게(~54px) 렌더링됨 -
      `globals.css`의 언레이어드 `button{ padding:9px 14px;
      border-radius:5px; ... }` 규칙이 Tailwind 유틸리티 클래스를 항상
      이기기 때문(이 세션에서 여러 번 반복된 알려진 함정). `padding`/
      `borderRadius`까지 인라인 `style`로 옮기고 나서야 의도한 크기로
      렌더링됨 - Playwright로 실측(수정 전 54px/항목 → 수정 후 6개 전부
      스크롤 없이 표시)까지 확인.
  - **`AdBannerPlaceholder`/`ChatPlaceholder` 신설**: 둘 다 좌측 메뉴
    컬럼 아래에 위치. 광고는 점선 테두리 버튼 하나("광고" 텍스트, 눌러도
    "아직 준비 중" 안내). 채팅은 배경 투명(별도 배경색 지정 안 함) +
    회색 placeholder 메시지 두 줄 + `disabled` 입력창 한 줄 - 처음엔
    `flex-1`을 줘서 사이드바 메뉴 영역과 세로 공간을 반반 나눠 먹는
    바람에 메뉴가 더 눌렸었는데, `shrink-0`(고정 높이)로 바꿔서 메뉴가
    우선적으로 공간을 갖도록 수정.
  - **출전 영웅 5슬롯을 세계 영역 아래로 이동**: 기존엔 세계 선택 레일
    위에 있었는데, 와이어프레임엔 세계 영역 아래(하단)에 있어서 순서만
    변경 - 로직/컴포넌트(`FormationSlot`)는 그대로.
  - 검증: `npx tsc --noEmit` 클린, `pnpm run build` 성공, `pnpm run
    test:game` 321/321(화면 리팩터라 게임 로직 변경 없음), 린트 기존
    문제 10개 제외 클린. Playwright로 위연 선택 후 전체 화면 스크린샷
    확인(사이드바 6개 항목 스크롤 없이 표시, 광고/채팅/출전영웅/세계
    카드 모두 겹침·오버플로우 없음 - `scrollHeight === clientHeight`
    실측 포함), `[연구]`/`[영웅]` 클릭 → 실제 화면 전환, `[병사]` 클릭 →
    "아직 준비 중" 안내까지 동작 확인.
- **영웅 클릭 시 뜨는 화면을 "정보 카드 + 리스트" 2분할로 재구성
  (2026-08-07)**: 사용자 요청 - "왼쪽에 영웅선택 화면의 표시되는
  영웅정보 화면이 그대로 표시되어야 해... < >를 눌러서 이전/다음 영웅",
  "오른쪽에는 영웅리스트가 보여야해... 선택하면 왼쪽 영웅정보가
  바뀌어야 해". 기존엔 별도의 두 화면(목록만 보여주는
  `HeroRosterScreen.tsx` / `HeroState` 기반 능력치·아이템 패널인
  `HeroDetailScreen.tsx`)이 순차 네비게이션으로 연결돼 있었는데, 이걸
  한 화면 안의 좌우 2분할로 합침.
  - **`HeroCard` 추출**: `HeroSelectScreen.tsx`(게임 시작 시 영웅 고르는
    화면)에만 있던 카드 컴포넌트(초상/등급뱃지/병과/능력치 5개/특기
    슬롯/스킬 팝업 버튼/설명)를 `app/game/HeroCard.tsx`로 그대로 옮겨서
    두 화면이 재사용. `selected`/`onSelect`를 둘 다 옵셔널로 바꿔서 -
    선택 화면은 기존처럼 3장 그리드에서 클릭으로 고르는 용도로 쓰고,
    새 로스터 화면은 `onSelect` 없이 읽기 전용으로 하나만 크게 표시.
  - **`HeroRosterScreen.tsx`를 목록+상세 통합 화면으로 재작성**: 왼쪽엔
    현재 선택된 영웅의 `HeroCard` + 하단에 `‹`/`›` prev/next 버튼과
    "N / 전체" 카운터(정렬 목록 기준 인덱스, 양끝에서는 버튼 비활성화 -
    순환 안 함). 오른쪽엔 기존 정렬 버튼(등급/유형/레벨순) + 목록을
    그대로 두되, 항목 클릭 시 더 이상 화면 전환이 아니라 내부
    `selectedId` state만 바꿔서 왼쪽 카드가 즉시 교체됨 - 선택된 항목은
    금색 테두리로 표시. 목록 행의 ★(출전 우선 토글)은 그대로 유지.
  - **`HeroDetailScreen.tsx` 삭제**: 능력치 진행바/경험치/아이템 장착
    UI가 있던 화면인데, `HeroCard`는 애초에 그 정보들을 안 보여주므로
    (사용자가 명시적으로 "영웅선택 화면과 그대로"를 요청) 이번엔 그
    표시를 옮기지 않음. 아이템 해제(`onUnequipItem`)도 같이 빠졌는데,
    확인해보니 실제로 아이템을 얻는 경로가 아직 전혀 없어서(테스트에서만
    `equipItem` 사용, `가방` 메뉴도 여전히 placeholder) 지금 당장은
    영향이 없음 - 나중에 아이템 시스템을 붙일 때 어디에 넣을지 다시
    결정하면 됨.
  - **`GameLobbyScreen.tsx` 진입 지점 통합**: 기존 `viewingHeroId`(개별
    영웅 직행)/`showHeroRoster`(목록 진입) 두 state를 `heroScreenOpen`
    + `heroScreenInitialId`로 합치고 `openHeroScreen(heroId | null)`
    헬퍼 하나로 정리. 출전 영웅 슬롯 클릭(채워진 슬롯/빈 슬롯 둘 다)과
    좌측 사이드바 `[영웅]` 메뉴가 전부 같은 통합 화면을 열되, 슬롯
    클릭만 그 영웅을 초기 선택값으로 넘김.
  - 검증: `npx tsc --noEmit` 클린, `pnpm run build` 성공, `pnpm run
    test:game` 321/321, 린트 기존 문제 10개 제외 클린. Playwright로
    위연으로 새 게임 → `[영웅]` 클릭 → 좌측에 `HeroCard`와 동일한 정보
    (통솔/무력/지력/체력/매력/특기 등) + "1 / 1" 카운터, 우측에 위연
    1명이 든 목록이 뜨는 것, 가로/세로 모두 오버플로우 없는 것
    (`scrollWidth === clientWidth`, `scrollHeight === clientHeight`),
    로비에서 출전 영웅 슬롯을 직접 클릭하면 그 영웅이 미리 선택된 채로
    같은 화면이 열리는 것까지 확인.
- **영웅 화면을 4열([영웅리스트][영웅정보][보물][가방])로 재배치 +
  보물/가방 시스템 자리 신설 (2026-08-07, 위 2분할 라운드 바로 다음)**:
  사용자가 리스트/정보 칸의 좌우 순서를 바꾸고, 그 오른쪽에 새 칸 둘을
  추가해달라고 요청.
  - **순서 변경**: 기존 [영웅정보(왼쪽)][영웅리스트(오른쪽)]를
    [영웅리스트][영웅정보]로 스왑 - 로직은 그대로, 두 `<div>` 블록
    순서만 바꿈.
  - **보물(신규 시스템) 자리 신설**: 사용자 확정 - "보물은 최대 4개까지
    영웅한테 장착 가능하고, 화면에서는 공간만 잡아줘 세로로, 보물은
    특기와 같은 능력을 주는 아이템". 기존 `hero.ts`의 범용
    `Item`/`MAX_ITEMS_PER_HERO`(2개)와는 별개의 새 시스템이고 사용자가
    "나중에 자세히" 정하겠다고 했으므로, `lib/game` 데이터 모델은 전혀
    안 건드리고 `MAX_TREASURE_SLOTS = 4`짜리 세로 잠김 슬롯 4개만
    화면에 예약(선택된 영웅과 무관하게 항상 잠김 - 장착 로직 없음).
  - **가방(보물 등 아이템 보관소) 자리 신설**: 사용자 확정 - "보물과
    같은 아이템을 보관하는 곳... 사용자 세력이 가지고 있는 아이템을
    보여주는 곳... 가로로 4칸 공간 잡아서... 아이템은 정사각형 형태로
    표시... 양이 많아지면 세로 스크롤". `Faction` 타입에 아이템 재고
    필드가 아직 없어서(확인함, `faction.ts`에 `items` 관련 필드 없음)
    지금은 `BAG_GRID_COLUMNS = 4`짜리 `grid-template-columns` 그리드에
    빈 정사각형 칸 4개 + "보유한 아이템이 없습니다" 안내만 표시 -
    그리드 컨테이너는 이미 `overflow-y-auto`라 나중에 아이템이 실제로
    쌓이면 자동으로 세로 스크롤됨.
  - 검증: `npx tsc --noEmit` 클린, `pnpm run build` 성공, `pnpm run
    test:game` 321/321(화면 전용 변경이라 게임 로직 영향 없음), 린트
    기존 문제 10개 제외 클린. Playwright로 4열이 844×390 박스 안에
    가로·세로 모두 오버플로우 없이 들어가는 것, 보물 4슬롯이 세로로,
    가방 4칸이 가로로 배치되는 것까지 스크린샷으로 확인.
- **영웅 화면 세부 조정 3건 (2026-08-07, 4열 재배치 바로 다음 라운드)**:
  1. **뒤로 버튼을 헤더 좌측 위로 이동**: `ScreenShell`의 `footer` prop을
     더 이상 안 쓰고, `header`를 `<h2>` 단독에서 `flex items-center
     gap-2`로 바꿔 `[뒤로]` 버튼 + "영웅" 제목을 한 줄에 배치. footer가
     차지하던 세로 공간이 그대로 본문(특히 영웅정보 칸)에 더해짐.
  2. **영웅리스트 폭 50% 증가**: `w-40`(160px) → `w-60`(240px). 늘린
     뒤에도 가운데 영웅정보 칸에 스크롤이 생기는지 Playwright로
     `scrollHeight`/`clientHeight`를 모든 `.overflow-y-auto` 요소에서
     실측해 확인했고, 50% 그대로 늘려도 스크롤이 안 생겨서 그대로
     적용(사용자가 "스크롤 생기면 덜 늘려도 된다"고 여지를 줬지만 이번엔
     안 줄여도 됐음).
  3. **가방 칸을 "+"/안내문에서 빈 바둑판 그리드로 교체**: 칸마다
     `(row + col) % 2`로 명암을 번갈아 칠하는 순수 CSS 체커보드 -
     아이콘이나 "보유한 아이템이 없습니다" 같은 문구 없이 빈 칸 자체로
     "여기가 아이템 슬롯"이라는 걸 보여줌. 미리 그려두는 칸 수는
     `BAG_EMPTY_PREVIEW_ROWS = 3`(4열×3행=12칸) - 실제 재고 데이터가
     여전히 없어서 몇 칸을 보여줄지는 순전히 시각적 선택.
  - 검증: `npx tsc --noEmit` 클린, `pnpm run build` 성공, `pnpm run
    test:game` 321/321, 린트 기존 문제 10개 제외 클린. Playwright로
    5개 `.overflow-y-auto` 컨테이너 전부 `scrollHeight === clientHeight`
    (스크롤 없음) 확인 + 스크린샷으로 세 가지 변경 모두 시각 확인.
- **영웅 화면 3영역을 시각적으로 구분 (2026-08-07, 위 세부 조정 바로
  다음 라운드)**: 사용자 요청 - "영웅 리스트, 영웅정보+보물, 가방
  영역이 구분이 되도록". 지금까지는 4개 칸이 `gap-2`만으로 떨어져 있어
  경계가 흐릿했는데, 이번엔 [영웅리스트] / [영웅정보+보물] / [가방]
  **3개 패널**로 묶어서 각각 테두리(`#25454f`)+배경(`#132a32`, 화면
  배경 `#10272e`보다 살짝 밝은 톤)이 있는 카드로 감쌈 - 영웅정보와
  보물은 "보물이 지금 선택된 영웅에게 장착하는 개념"이라 한 패널 안에
  나란히 두고 그 사이에만 얇은 세로 구분선(`border-l`)을 그음.
  - 패널에 `p-1.5` 안쪽 여백이 새로 생겨서 각 칸의 스크롤 가능 영역이
    조금씩 줄었는데, Playwright로 재확인한 결과 그래도 스크롤은 여전히
    안 생김(직전 라운드에서 이미 여유가 있었던 덕).
  - 검증: `npx tsc --noEmit` 클린, `pnpm run build` 성공, `pnpm run
    test:game` 321/321, 린트 기존 문제 10개 제외 클린. Playwright로
    5개 `.overflow-y-auto` 컨테이너 전부 스크롤 없음 재확인 + 스크린샷
    으로 3패널 구분이 잘 보이는 것 확인.
- **영웅리스트 행을 3줄 정보 + 확대 초상으로 재구성 (2026-08-07, 위 패널
  구분 라운드 바로 다음)**: 사용자가 예시(`위연 [출전]` / `[B급] Lv1` /
  `장군형 적벽촌 영주`)까지 붙여서 요청.
  - **1줄**: 이름 + 출전 배지. 기존엔 행 맨 앞에 별도의 ★/☆ 버튼(작은
    별 아이콘)이 있었는데, 이걸 없애고 이름 옆에 "출전"이라고 적힌 네모
    배지로 교체 - `onToggleDeploymentPriority` 클릭 핸들러는 그대로
    이 배지로 옮김(꺼진 상태도 계속 표시해서 눌러 켤 수 있게 함, 켠
    상태만 금색으로 강조).
  - **2줄**: 등급 배지를 텍스트("B급 · 장군형 · Lv.1" 한 줄)에서
    분리해 **등급색으로 채워진 네모박스**("B급") + "Lv.1"로 나눔 -
    `GRADE_COLOR[grade]`를 배경색으로, 짙은 남색(`#0b2028`) 글자로 채워
    HeroCard의 테두리만 있는 등급 뱃지와는 다른, 더 진한 "색 칩" 느낌.
  - **3줄**: 유형(장군형 등) + 영주 라벨(있을 때만, 기존과 동일 데이터).
  - **초상 확대**: 텍스트가 2줄 → 3줄로 늘어난 만큼 `h-8/w-8`(32px) →
    `h-12/w-12`(48px)로 비례 확대.
  - 검증: `npx tsc --noEmit` 클린, `pnpm run build` 성공, `pnpm run
    test:game` 321/321, 린트 기존 문제 10개 제외 클린. Playwright로
    5개 `.overflow-y-auto` 컨테이너 전부 스크롤 없음 확인(3줄로 늘어난
    행 높이가 리스트 패널 폭·초상 확대와 함께 들어가는지) + 스크린샷
    으로 실제 렌더링(이름/출전 배지/등급 칩/레벨/유형이 예시와 같은
    배치로 나오는 것) 확인.
- **`HeroCard.tsx`를 목업대로 재설계: 세로 능력치 1열 + 확대 초상 + 신규
  "Point" 박스 (2026-08-07, 위 리스트 3줄 라운드 바로 다음)**: 사용자가
  손그림 목업(위연/[B급*장군형]/병과 보병 헤더, 큰 세로 초상 옆에
  통솔~매력 세로 나열, 그 아래 "Point 20 ↑" 박스, 특기 슬롯은 잠김
  아이콘 대신 "-")을 첨부하며 "중간의 영웅 정보를 첨부한 내용처럼
  적용해줘"라고 요청. `HeroCard.tsx`는 `HeroSelectScreen`(시작 시
  3장 그리드)과 `HeroRosterScreen`(영웅 화면 가운데 칸) 둘 다 쓰는
  **공유 컴포넌트**라 이번 변경은 두 화면 모두에 적용됨 - 사용자가 예전에
  "왼쪽 정보는 영웅선택 화면과 그대로 동일해야 해"라고 직접 확정했던
  원칙과도 일치.
  - **능력치를 `grid-cols-2`에서 세로 1열(`flex flex-col`)로 변경** -
    통솔/무력/지력/체력/매력 순서 그대로, 한 줄에 하나씩.
  - **초상 프레임을 정사각형(96×96)에서 세로로 긴 직사각형으로 확대** -
    `HeroCard.tsx`에 `PORTRAIT_WIDTH_PX`/`PORTRAIT_HEIGHT_PX`(현재
    84×100) 신설, 기존 공용 `HERO_PORTRAIT_FRAME_PX` 상수는 이제 이
    파일에서 안 써서 삭제(다른 화면은 각자 파일에서 원형 아바타 크기를
    직접 지정하고 있었음, `heroPortraits.ts` 문서 주석도 갱신). **동적
    높이(items-stretch)가 아니라 고정 width+height를 유지** - 이 파일의
    예전 기록(실제 아트 이미지가 stretch된 flex item 안에서 원본 비율로
    쪼그라드는 현상)을 다시 겪지 않기 위해.
  - **신규 "Point" 박스**: 목업에만 등장한 새 개념(추정: 능력치를 올리는
    데 쓰는 포인트) - 레벨업 시 포인트를 얼마나/어떻게 주는지 등 실제
    시스템이 전혀 없어서(`lib/game/hero.ts`의 `attributeProgress`가
    "pip을 채우는 로직 자체가 아직 미정"이라고 이미 기록돼 있음) 이번엔
    **화면 자리만 예약**: 항상 "0" 고정 표시 + `↑` 버튼은 눌러도 "아직
    준비 중" 안내만.
  - **특기 빈 슬롯 표시를 🔒 → "-"로 변경** (목업 그대로) - 스킬 팝업의
    빈 슬롯 잠금 아이콘(🔒)은 이 요청 범위 밖이라 그대로 둠.
  - **간격/폰트 조정**: 능력치가 3행(2열 grid, 5개→3행)에서 5행(1열)으로
    늘고 Point 박스까지 추가되면서 카드 전체 높이가 844×390 박스 안에서
    스크롤 없이 안 들어갔음(첫 시도 기준 +64px 초과) - `gap`/폰트 크기/
    패딩을 `text-sm`→`text-xs`, `gap-1`→`gap-0.5`, 섹션 사이 `mt-1.5`→
    `mt-1`/`mt-0.5` 등으로 촘촘하게 줄여서 스크롤 없이 맞춤(설명 문단
    줄 수는 그대로 3줄 유지 - 내용을 자르지 않고 간격만으로 해결).
  - 검증: `npx tsc --noEmit` 클린, `pnpm run build` 성공, `pnpm run
    test:game` 321/321(화면 전용 변경), 린트 기존 문제 10개 제외 클린.
    Playwright로 (1) 영웅 화면의 5개 `.overflow-y-auto` 컨테이너 전부
...
- **영웅 화면 크기/폭 미세 조정 5건 (2026-08-07, 위 분리 라운드 바로
  다음)**: 사용자 요청 - "영웅리스트 가로폭을 좀 줄이고, 영웅정보의
  영웅 이미지 크기를 좀 더 키우자. 등급 폰트도 좀 더 키우고 영웅정보
  화면은 능력을 세로로 나열했기 때문에 가로 폭을 좀더 줄이고 우측에
  있는 가방의 칸을 보물 칸 만큼 늘려줘". 전부 `HeroRosterScreen.tsx`/
  `HeroInfoPanel.tsx`만 건드림 - 방금 분리해둔 덕에 `HeroCard.tsx`(새
  게임 영웅선택 화면)는 전혀 안 건드려도 됨.
  - **영웅리스트 패널**: `w-60`(240px) → `w-48`(192px).
  - **가방 패널**: `w-40`(160px) → `w-60`(240px) - 칸 크기 자체를
    정하는 상수는 없고 4열 그리드가 이 패널 폭을 나눠 채우는 구조라,
    패널을 넓히는 것만으로 칸 하나 크기가 보물 슬롯(`w-16` 컬럼 안의
    정사각형)과 비슷해짐.
  - **영웅정보 칸 폭**: 리스트+가방이 넓어진 만큼 자동으로 줄어듦(둘 다
    고정폭, 영웅정보+보물을 담은 가운데 패널만 `flex-1`이라 나머지
    공간을 그대로 물려받음) - 능력치가 세로 1열이라 가로 공간이 예전만큼
    필요 없다는 사용자 설명과도 맞음.
  - **영웅 이미지(초상) 확대**: `HeroInfoPanel.tsx`의
    `PORTRAIT_WIDTH_PX`/`PORTRAIT_HEIGHT_PX`를 84×100 → 94×112로.
  - **등급 폰트 확대**: 상단 "B급·장군형" 배지(`text-xs`→`text-sm`) +
    능력치 각 줄의 등급 글자(`GradeStat`, 값만 `text-sm`으로 - 라벨은
    `text-xs` 그대로 둬서 공간 절약).
  - 리스트/가방 패널이 넓어지면서 영웅정보 칸이 좁아지고, 초상+등급
    폰트까지 커지면서 카드 세로 높이가 다시 스크롤 경계를 넘었음(처음
    +22px 초과) - `dl`의 `gap-0.5`→`gap-0`, Point 박스/특기 슬롯
    패딩을 `py-0.5`→`py-px`, 카드 자체 패딩·섹션 사이 마진을 한 번 더
    촘촘히 줄여서(설명 문단 줄 수는 그대로 3줄 유지) 스크롤 없이 맞춤 -
    Playwright로 실측하며 1px 단위로 좁혀감.
  - 검증: `npx tsc --noEmit` 클린, `pnpm run build` 성공, `pnpm run
    test:game` 321/321, 린트 기존 문제 10개 제외 클린. Playwright로
    5개 `.overflow-y-auto` 컨테이너 전부 스크롤 없음 확인 + `HeroCard`
...
- **병사/연구 화면 분리 + 병과 트리를 "좌측 계열 → 우측 진화 단계" 표로
  재구성 (2026-08-08, 위 항목 바로 다음 라운드)**: 사용자 요청 두 가지를
  이어서 반영.
  1. **화면 역할 분리**: "병사 화면은 지금 선택 가능한 병사를 확인하고
     출전할 병과를 고르는 용도, 연구 화면은 레벨업/진화(해금)만" -
     `ResearchScreen.tsx`에서 "출전" 버튼(`onSetActive`)을 빼고
     `app/game/screens/TroopsScreen.tsx`를 신설해 로비의 기존 "병사" 메뉴
     항목(그동안 자리만 있고 미구현이던 스텁)에 연결. 해금 안 된 병과도
     🔒로 함께 보여주되 지금은 클릭해도 반응 없음 - "클릭하면 연구
     화면으로 넘어가 해금" 기능은 사용자가 명시적으로 "나중에 추가"로
     미룸(주석에 남겨둠).
  2. **트리를 표 형태로 재구성**: "좌측에 기본 병과 계열별로 표시하고
     우측으로 갈수록 진화된 병과 표시" - 사용자가 준 예시(보병-검병-
     산악병-..., 궁병-석궁병-화차-충차-발석차-..., 기병-철기병-,
     책사-참모-)를 그대로 채택. 두 가지 구조 변경이 따라옴:
     - **공성(siege) 계열을 없애고 궁병 계열의 상위 단계로 흡수** -
       사용자 예시에 공성이 따로 없고 대신 궁병 줄에 화차/충차/발석차 같은
       공성 병기 이름이 있었음. `TroopLine`이 5종 →
       `"infantry"|"archer"|"cavalry"|"strategist"` 4종으로 줄고,
       `unit-production.ts`의 옛 `siege`/`siege_catapult` 노드는 삭제.
     - **분기(같은 단계에 여러 후보)를 없애고 계열당 외길로 단순화** -
       "분기는 나중에 추가할께"라는 사용자 확인에 따라, 이전 라운드에
       넣었던 분기/교차 계열 선행조건/아이템 해금 데모 노드(`infantry_
       shield`/`infantry_ranger`/`cavalry_dragoon`의 교차 계열 조건/
       `strategist_water`)를 전부 걷어내고, 계열마다 `tier` 0(기본)~5(5단계)
       6개 노드가 일렬로 이어지는 사슬로 재작성. `UnitTypeNode`에
       `tier: number` 필드 추가 - 표의 열 위치를 그대로 결정. `unitTypesInLine`이
       이제 tier 순으로 정렬해서 반환. `UnitTypeUnlock`의
       `requiredItemId`/`requiredUnitTypeIds` 필드 자체(및
       `unit-evolution.ts`의 관련 로직)는 분기가 돌아올 때 쓸 인프라로
       타입만 남겨두고 지금 카탈로그 데이터에서는 안 씀.
  - **진화 단계 이름은 사용자 요청으로 이번 세션이 추천, 사용자가 그대로
    채택**: 보병(보병-검병-산악병-중보병-철갑보병-근위보병), 궁병(궁병-
    석궁병-화차-충차-발석차-벽력거), 기병(기병-철기병-중기병-창기병-
    용기병-비룡기병), 책사(책사-참모-군사-방사-도사-신선, 군사/방사/도사/
    신선은 각각 제갈량의 실제 직함·중국 고전 술법가 용어·도교 사제·
    불사의 경지에서 따온 명명 - draft, 실제 최종 이름은 아님). 해금
    비용은 `tierUnlock(tier)` 헬퍼로 tier가 깊을수록 등급 조건(C→B→A)과
    금/유산 비용이 함께 오르게 통일.
  - 영향받은 기존 파일: `legendary-heroes.ts`(관우의 진화 데모 대상이
    `cavalry_heavy` -> `cavalry_ironclad`로, tier 구조에 맞게 재배치),
    `hero-definition.test.ts`/`combat.test.ts`(옛 `"siege"` unitType
    참조를 `"archer"`/`"strategist"`로 교체), `unit-production.test.ts`
    (`siege` -> `strategist`), `unit-evolution.test.ts`(분기/교차계열/
    아이템 테스트를 새 외길 구조에 맞게 다시 씀).
  - **버그 하나 잡음**: `TroopsScreen.tsx`의 표 칸을 raw `<button>`으로
    처음 만들었더니 `app/globals.css`의 unlayered `button {...}` 전역
    규칙(금색 그라데이션 배경)이 Tailwind 클래스를 다 이겨서 칸이 전부
    금색으로 보이는 문제가 있었음 - `Button.tsx` 주석에 이미 문서화된
    "cascade layers, 어떤 specificity로도 못 이김" 케이스라, 같은 방식대로
    배경/테두리/패딩/글자색을 전부 인라인 `style`로 옮겨서 해결(`cellStyle`
    헬퍼). `ResearchScreen.tsx`의 칸은 `<div>` + 기존 `Button` 컴포넌트
    조합이라 처음부터 문제 없었음.
  - 검증: `npx tsc --noEmit` 클린, `pnpm run build` 성공, `pnpm run
    test:game` 339/339, 린트 클린. Playwright로 병사/연구 화면 둘 다
    실제 렌더링 확인(표 헤더 "기본~5단계", 좌측 계열 라벨, 잠금
    칸 스타일까지 스크린샷으로 검증).
