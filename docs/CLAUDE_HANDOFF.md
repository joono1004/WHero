...
### 로비 화면 재배치 1차 초안 - 지형 아트 자리 예약 (2026-08-06)

**Codex가 나중에 세계(지역) 지형 아트를 만들 때 필요한 부분만 요약**
(배경/결정 과정 전체는 `docs/SYSTEM_LAYER.md`의 "로비 화면 재배치 1차
초안 + 영주 임명 시스템 신설" 항목 참고):

- 로비 화면(`app/game/screens/GameLobbyScreen.tsx`)의 세계 레일(클리어한
  세계 히스토리 + 새로 도전할 세계 후보 카드)에 **지형 아트가 들어갈
  자리를 `TerrainArtPlaceholder` 컴포넌트로 예약해뒀습니다.** 지금은
  "지형"이라는 텍스트만 있는 점선 정사각형 박스입니다 - 실제 지형
  이미지/썸네일은 전혀 없습니다.
- 박스 크기는 `56 + (해당 맵 티어의 MAP_TIER_ORDER 인덱스) * 8`px로,
  미니(0단계)는 56px, 월드(6단계, 최고 티어)는 104px까지 커집니다 - "정복한
  세계가 많을수록 큰 세계가 보인다"는 사용자 방향을 실제 아트 없이도
  어느 정도 시각적으로 드러내려는 임시 조치입니다. 실제 아트가 들어오면
  이 스케일링 로직도 다시 봐야 할 수 있습니다(고정 프레임 + 아트 자체가
  더 웅장하게 그려지는 방식이 나을 수도 있음).
- 어떤 지형 종류(`MapTypeId`: `inland`/`continent`/`archipelago`/
...
### 병과 트리 표 재구성 - 공성 계열 제거 (2026-08-08, 위 항목 바로 다음
  라운드, `main`에 직접 병합)

**Codex가 알아야 할 부분만 요약** (배경/결정 과정 전체는
`docs/SYSTEM_LAYER.md`의 "병사/연구 화면 분리 + 병과 트리를 표로 재구성"
항목 참고). 이번에도 사용자가 채팅에서 직접 승인해서 `main`에 바로
병합했습니다(같은 예외 패턴).

- **`TroopLine`에서 `"siege"`가 없어졌습니다** - 5계열 → 4계열
  (`infantry`/`archer`/`cavalry`/`strategist`). 사용자가 "좌측 계열,
  우측으로 진화" 표를 요청하면서 준 예시에 공성이 따로 없고 궁병 계열
  상위 단계(화차/충차/발석차/벽력거)로 흡수됐기 때문입니다. Codex 쪽에서
  혹시 `"siege"`라는 병과 이름/id를 지형·유닛 배치 로직에서 참조하고
  있었다면 더 이상 존재하지 않으니 확인이 필요합니다 - 대신 `archer`
  계열의 `archer_fire_cart`/`archer_ram`/`archer_trebuchet`/
  `archer_thunder_cart`(tier 2~5)가 그 자리를 대신합니다.
  `unit-production.ts`의 `UNIT_TYPE_CATALOG`가 최신 출처입니다.
  - **분기(같은 단계에 여러 후보 병과)와 교차 계열 선행조건도 이번에
    걷어냈습니다** - 사용자가 "분기는 나중에 추가"로 명시적으로 미룸.
    지금은 계열마다 `tier` 0~5(`UnitTypeNode.tier`)가 외길로 이어지는
    구조뿐입니다. 나중에 분기가 돌아오면 카탈로그 노드 하나에 여러
    `parentId`가 같은 자식이 붙는 형태로 다시 확장될 수 있습니다.
